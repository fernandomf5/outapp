...
import { DollarSign, Package, Shield, ShoppingCart, CheckCircle2, Star, CreditCard, Lock, Smartphone, Heart, Clock, Pencil, RefreshCcw } from "lucide-react";
            ...
            <Card className={`border shadow-sm p-6 space-y-4 group relative ${cardRadius} ${cardShadow} ${effectClasses} ${activeTab === 'form' || activeTab === 'payment' ? 'ring-2 ring-green-500 ring-offset-4 ring-offset-slate-900 animate-pulse' : ''}`} style={{ backgroundColor: checkout.custom_settings?.card_color || '#ffffff', borderColor: borderColor }}>
              ...
              {activeTab !== 'payment' ? (
                ...
                <div className="space-y-4">
                  ...
                  <Button className="w-full h-12 font-bold rounded-xl shadow-lg" style={{ backgroundColor: primaryColor, color: checkout.button_text_color || textColor === '#ffffff' ? '#ffffff' : (textColor || '#ffffff') }}>
                    Finalizar Pagamento
                  </Button>
                </div>
              )}
