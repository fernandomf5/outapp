...
export const VideoCover = ({ videoUrl, logoUrl }: VideoCoverProps) => {
  ...
  return (
    <div className="p-1 sm:p-1.5">
      <div
        className="relative mx-auto aspect-video w-full cursor-pointer overflow-hidden rounded-xl sm:rounded-2xl md:rounded-3xl shadow-2xl shadow-black/20 group laser-border bg-background"
        onClick={handlePlay}
        onKeyDown={(event) => {
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute -top-10 -left-10 w-44 sm:w-72 h-44 sm:h-72 rounded-full bg-primary/25 blur-[70px] animate-pulse" />
          <div className="absolute top-1/2 -right-12 w-36 sm:w-56 h-36 sm:h-56 rounded-full bg-primary-glow/30 blur-[55px] animate-pulse" style={{ animationDelay: '1s' }} />
          <div className="absolute -bottom-8 left-1/3 w-32 sm:w-48 h-32 sm:h-48 rounded-full bg-primary/20 blur-[50px] animate-pulse" style={{ animationDelay: '0.5s' }} />
        </div>

        {/* Inner glass card */}
        <div className="absolute inset-[4px] rounded-[calc(var(--radius)-1px)] bg-card/50 backdrop-blur-sm border border-primary/15" />
        ...
        <div className="relative z-10 flex flex-col items-center justify-center h-full text-white px-4 py-6">
          ...
          <img
            ...
            alt="Out App"
            className="w-16 h-16 xs:w-20 xs:h-20 sm:w-28 sm:h-28 md:w-32 md:h-32 object-contain mb-3 xs:mb-4 sm:mb-5 drop-shadow-2xl animate-fade-in"
          />

          {/* Play button */}
          <div className="relative mb-3 xs:mb-4 sm:mb-5">
            {/* Pulse rings */}
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-16 h-16 xs:w-20 xs:h-20 sm:w-28 sm:h-28 md:w-32 md:h-32 rounded-full bg-primary/35 animate-ping" style={{ animationDuration: '2s' }} />
            </div>
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-14 h-14 xs:w-16 xs:h-16 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-full bg-primary-glow/25 animate-ping" style={{ animationDuration: '2s', animationDelay: '0.5s' }} />
            </div>

            {/* Play button */}
            <div className="relative w-14 h-14 xs:w-16 xs:h-16 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-full bg-white/10 backdrop-blur-md border-2 border-primary/70 flex items-center justify-center group-hover:scale-110 group-hover:bg-primary/25 transition-all duration-300 shadow-[0_0_30px_hsl(var(--primary-hsl,_116_62%_38%)/0.45)]">
