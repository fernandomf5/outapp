...
export const VideoCover = ({ videoUrl, logoUrl }: VideoCoverProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="relative z-10 flex flex-col items-center justify-center h-full text-white px-3 py-4">
        {/* Logo */}
        <img 
          src={logoUrl || outAppLogo} 
          alt="Out App" 
          className="w-16 h-16 xs:w-20 xs:h-20 sm:w-32 sm:h-32 md:w-40 md:h-40 object-contain mb-3 xs:mb-4 sm:mb-8 drop-shadow-2xl animate-fade-in"
        />
        ...
        <div className="relative mb-3 xs:mb-4 sm:mb-8">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-16 h-16 xs:w-20 xs:h-20 sm:w-28 sm:h-28 md:w-32 md:h-32 rounded-full bg-white/20 animate-ping" style={{ animationDuration: '2s' }} />
          </div>
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-14 h-14 xs:w-16 xs:h-16 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-full bg-white/10 animate-ping" style={{ animationDuration: '2s', animationDelay: '0.5s' }} />
          </div>
          
          {/* Play button */}
          <div className="relative w-14 h-14 xs:w-16 xs:h-16 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-full bg-white/20 backdrop-blur-sm border-2 border-white/40 flex items-center justify-center group-hover:scale-110 group-hover:bg-white/30 transition-all duration-300 shadow-2xl">
            <Play className="w-6 h-6 xs:w-7 xs:h-7 sm:w-10 sm:h-10 md:w-12 md:h-12 text-white fill-white ml-0.5" />
          </div>
        </div>

        {/* Text */}
        <p className="text-base xs:text-lg sm:text-2xl md:text-3xl lg:text-4xl font-bold tracking-wide animate-fade-in drop-shadow-lg">
          Aperte o Play
        </p>
        <p className="text-xs xs:text-sm sm:text-base md:text-lg text-white/80 mt-1 xs:mt-2 sm:mt-3 animate-fade-in" style={{ animationDelay: '0.2s' }}>
