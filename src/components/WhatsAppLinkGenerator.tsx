...
export const WhatsAppLinkGenerator = () => {
  ...
  const generateScript = () => {
    const cleanPhone = phone.replace(/\D/g, '');
    const encodedMessage = encodeURIComponent(message);
    const whatsappLink = `https://wa.me/${cleanPhone}${message ? `?text=${encodedMessage}` : ''}`;
...
<a id="whatsapp-float-btn" href="${whatsappLink}" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-accent/50 p-4 rounded-lg">
              ...
              <div className="relative">
                <pre className="bg-card p-4 rounded border w-full max-w-full overflow-x-auto max-h-[300px] overflow-y-auto text-xs whitespace-pre-wrap break-words font-mono">
                  {generateScript()}
                </pre>
                <Button
