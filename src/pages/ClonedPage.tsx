  ...
  useEffect(() => {
    ...
    try {
      ...
      if (settings.whatsapp_button?.enabled && settings.whatsapp_button?.phone) {
        const body = doc.querySelector('body');
        const cleanPhone = settings.whatsapp_button.phone.replace(/\D/g, '');
        const whatsappLink = `https://wa.me/${cleanPhone}${settings.whatsapp_button.message ? '?text=' + encodeURIComponent(settings.whatsapp_button.message) : ''}`;
        const whatsappHtml = `
          <style>
            #whatsapp-cloned-btn {
              position: fixed;
              ${getWhatsAppPosition(settings.whatsapp_button.position)};
          ...
          <script>
            (function() {
              ...
              var whatsappBtn = document.getElementById('whatsapp-cloned-btn');
              if (whatsappBtn) {
