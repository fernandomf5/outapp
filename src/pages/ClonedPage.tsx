  ...
  useEffect(() => {
    ...
    try {
      ...
      if (settings.whatsapp_button?.enabled && settings.whatsapp_button?.phone) {
        ...
        const message = settings.whatsapp_button.message ? `?text=${encodeURIComponent(settings.whatsapp_button.message)}` : '';
        const whatsappLink = `https://wa.me/${cleanPhone}${message}`;
        const whatsappHtml = `
          <style>
            #whatsapp-cloned-btn {
              position: fixed;
