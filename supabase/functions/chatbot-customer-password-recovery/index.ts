...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { action, email, chatbotId, resetToken, newPassword } = await req.json();

    if (action === "reveal") {
      // Buscar cliente pelo email e chatbot
      ...
      await resend.emails.send({
        ...
        to: [email],
        subject: `Sua senha - ${chatbot?.name || "Chat Online"}`,
        html: `
          <!DOCTYPE html>
          <html>
          <head>
            ...
            <style>
              ...
              .password-box { background: white; border: 2px solid #2563eb; padding: 20px; text-align: center; margin: 20px 0; }
          ...
          <body>
            <div class="container">
              ...
              <div class="content">
                ...
                <p>Você solicitou a recuperação de senha para o <strong>${chatbot?.name || "Chat Online"}</strong>.</p>
                <div class="password-box">
                  <p style="margin: 0; font-size: 14px; color: #666;">Sua senha é:</p>
                  <div class="password">${customer.password_hash}</div>
                </div>
                <p style="color: #dc2626; font-weight: bold;">⚠️ Guarde esta senha em local seguro!</p>
      ...
      return new Response(
        JSON.stringify({ success: true, message: "Senha enviada para o e-mail" }),
        ...
        html: `
          ...
          <body>
            <div class="container">
              ...
              <div class="content">
                <p>Olá, <strong>${customer.name}</strong>!</p>
                <p>Você solicitou a redefinição de senha para o <strong>${chatbot?.name || "Chat Online"}</strong>.</p>
