...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (action === "reveal") {
      // Buscar cliente pelo email e agente
      ...
      await resend.emails.send({
        ...
        to: [email],
        subject: `Sua senha - ${agent?.name || "Chat do Agente IA"}`,
        html: `
          <!DOCTYPE html>
          <html>
          <head>
          ...
          <body>
            <div class="container">
              ...
              <div class="content">
                ...
                <p>Você solicitou a recuperação de senha para o <strong>${agent?.name || "Chat do Agente IA"}</strong>.</p>
                <div class="password-box">
                  <p style="margin: 0; font-size: 14px; color: #666;">Sua senha é:</p>
                  <div class="password">${customer.password}</div>
                </div>
    ...
    if (action === "request-reset") {
      ...
      const resetUrl = `${Deno.env.get("APP_URL") || "https://mlocikcfxbleddsvxciv.supabase.co"}/agent-reset-password/${token}`;
